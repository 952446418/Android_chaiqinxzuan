...
//        });
        ...
        mBtnDownloadItem.setOnClickListener(new View.OnClickListener( ) {
            ...
            public void onClick(View v) {
                ...
                //从SQLite数据库中读出数据。
                dbHelper = new MyDatabaseHelper(DeleteActivity.this, "cqx", null);
                List<Student> students = query(dbHelper.getReadableDatabase( ));

                HSSFWorkbook mWorkbook = new HSSFWorkbook( );
                HSSFSheet mSheet = mWorkbook.createSheet(MyDatabaseHelper.TABLE_NAME);
            ...
            private ContentValues getContentValues(String MedicineName, Float TotalNumber, Float Value, Float Min) {
                ContentValues contentValues = new ContentValues( );
                contentValues.put(MyDatabaseHelper.ITEM_MedicineName, MedicineName);
                contentValues.put(MyDatabaseHelper.ITEM_TotalNumber, TotalNumber);
                contentValues.put(MyDatabaseHelper.ITEM_Value, Value);
                contentValues.put(MyDatabaseHelper.ITEM_Min, Min);
            ...
            private List<Student> query(SQLiteDatabase db) {
                ...
                if (cursor != null && cursor.getCount( ) > 0) {
                    ...
                    while (cursor.moveToNext( )) {
                        ...
                        student.id = cursor.getInt(cursor.getColumnIndex(MyDatabaseHelper.ITEM_ID));
                        student.name = cursor.getString(cursor.getColumnIndex(MyDatabaseHelper.ITEM_MedicineName));
                        student.gender = cursor.getString(cursor.getColumnIndex(MyDatabaseHelper.ITEM_TotalNumber));
                        student.age = cursor.getInt(cursor.getColumnIndex(MyDatabaseHelper.ITEM_Value));
                        student.age = cursor.getInt(cursor.getColumnIndex(MyDatabaseHelper.ITEM_Min));
            ...
            private void createExcelHead(HSSFSheet mSheet) {
                ...
                headRow.createCell(0).setCellValue(MyDatabaseHelper.ITEM_ID);
                headRow.createCell(1).setCellValue(MyDatabaseHelper.ITEM_MedicineName);
                headRow.createCell(2).setCellValue(MyDatabaseHelper.ITEM_TotalNumber);
                headRow.createCell(3).setCellValue(MyDatabaseHelper.ITEM_Value);
                headRow.createCell(3).setCellValue(MyDatabaseHelper.ITEM_Min);
            }

            // 创建Excel的一行数据。
            private void createCell(int id, String name, String gender, int age, HSSFSheet sheet) {
